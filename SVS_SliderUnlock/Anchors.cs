...
internal class Anchors
{
    ...
    {
        long Search(string[] patterns, long seekPos = -1, long assert = -1)
        {
            ...
            throw new Exception($"non of patten in {string.Join(", ", patterns)} found");
        }
        ...
        IsFace_BranchGreaterThanOne = Search(
            [
                // 0x18067db28 SVS v1.1.6
                "7f ?? ?? 8b 47 38 89 75 7f ?? 8b 00 f6 80 32 01 00 00 01 75 ?? ?? 8b c8 e8 ?? ?? ?? ?? ?? 8d 55 7f ?? 8b c8 e8",
                // 0x180680518
        ...
        IsFace_BranchLessThanZero = Search(
            [
                // 0x18067db6a SVS 1.1.16
                "0f 85 ?? ?? ?? ?? ?? 85 f6 74 ?? ?? 8b 0d ?? ?? ?? ?? 39 b1 e0 00 00 00 75 ?? e8",
                // 0x18068055b
